...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[GefgCurFormat]') and xtype in (N'FN', N'IF', N'TF'))
drop function [dbo].[GefgCurFormat]
GO


CREATE Function GefgCurFormat
(
	@Number Numeric(27,7),
...
As
BEGIN
-- select dbo.gefgGetCmpCnfgVal('gCurrDigit',1)
-- select dbo.gefgGetCmpCnfgVal('gQtyDigit',1)
	
	Declare @Number1 Numeric(27,7)  			--SJ29062007
	Declare @RoundTo TinyInt 
	SELECT @CmpFK =Cmppk from GenCmpMas (NOLOCK) where CmpDelid=0
	SELECT @RoundTo =  dbo.gefgGetCmpCnfgVal('gCurrDigit',@CmpFK)		--SJ09022007	
	SET @Number1 = @Number
	
	SELECT @RoundTo =  IsNull(@RoundTo,4)


	if @Number < 0 							--SJ29062007
		Set @Number = @Number*-1
	else
		Set @Number = @Number
	...
	--SELECT @DeciPart  = Right(@Numb2Char,Len(@Numb2Char)-CharIndex('.',@Numb2Char))
	...
	END
	if @Number1 < 0 Set @Result = '-' + @Result		--SJ29062007
	Return left(@Result,len(@Result)-1)+@DeciPart		-- SBN23112011
--	SELECT left(@Result,len(@Result)-1)+ @DeciPart
END
