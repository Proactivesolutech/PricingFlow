ALTER PROC dbo.PrcCalculateLoan
(
	@LEADPK		BIGINT,
	@PrdCode	VARCHAR(20)
)
...
BEGIN 
	...
	DECLARE @OutSandAmt NUMERIC(27,2) = 0,@ARR_OutSandAmt NUMERIC(27,2) = 0 , 
		...
		SELECT @FOIR = CASE WHEN @INCOME > 30000 THEN 60 ELSE 50 END
		-- FOIR AMOUNT

		DECLARE @FOIR_AMT NUMERIC(27,2) = (ISNULL(@INCOME,0) * (@FOIR / 100)) - @OBLIGATIONS;
		...
		IF @INCOME <> 0
			SELECT @ARR_FOIR  = ((@TEMP_EMI + @OBLIGATIONS) / @INCOME) * 100;
		ELSE
			SELECT @ARR_FOIR = 0

			
			--SELECT 'SELECT ' , @SALARY_TYPE , @SAL_PROOF , @ARR_LTV , @CIBIL , @ARR_FOIR

		-- ARRIVED ROI
		DECLARE @ARR_ROI NUMERIC(27,2) ;		
		SELECT  @ARR_ROI  = dbo.fnCalcROI(@SALARY_TYPE , @SAL_PROOF , @ARR_LTV , @CIBIL , @ARR_FOIR );
		...
		IF @INCOME <> 0
			SELECT @FIN_ARR_FOIR = ((@ARR_EMI + @OBLIGATIONS) / @INCOME) * 100;
		ELSE
			SELECT @FIN_ARR_FOIR = 0


		-- IF the product is BT+Topup or Topup 
		IF @PrdCode = 'HLBTTopup' OR @PrdCode = 'HLTopup'
		BEGIN
			...
			IF (@PrdCode = 'HLBTTopup') 
            BEGIN
				IF @INCOME <> 0
					SELECT @FIN_ARR_FOIR = ((@BT_EMI + @TOPUP_EMI + @OBLIGATIONS) / @INCOME) * 100;
				ELSE
				SELECT @FIN_ARR_FOIR = 0
            END
		...
		IF @PrdCode IN ('HLExt','HLImp') 
		BEGIN
			SET @ARR_OutSandAmt = ISNULL(@OutSandAmt,0)
			IF @ACT_PRP_VAL <> 0
			BEGIN				
				SELECT @TOPUP_LTV_A  = (ISNULL(ROUND(@FINAL_LOAN_AMT,0),0) / @ACT_PRP_VAL ) * 100 ;
				SELECT @LTV_PERC_A = (ISNULL(ROUND(@FINAL_LOAN_AMT + @ARR_OutSandAmt,0),0) / @ACT_PRP_VAL ) * 100 ;
			END
			ELSE
			BEGIN	
				SELECT @TOPUP_LTV_A  = 0
			...
			IF @MRKPROP_VAL <> 0
			BEGIN
				SELECT @TOPUP_LTV_M  = (ISNULL(ROUND(@FINAL_LOAN_AMT,0),0) / @MRKPROP_VAL ) * 100 ;
				SELECT @ARR_LTV = (ISNULL(ROUND(@FINAL_LOAN_AMT+@ARR_OutSandAmt,0),0) / @MRKPROP_VAL ) * 100 ;
			END
			ELSE
			BEGIN	
				SELECT @TOPUP_LTV_M  = 0
		...
		IF @IsPNI = 'Y'
		BEGIN
			IF @PrdCode IN ('HLNew' , 'HLResale','HLConst','HLPltConst')
			BEGIN
				IF @SAL_PROOF = 0
					SELECT @ARR_LTV = 80 ,@LTV_PERC_A = 80, @LTV_PERC=80
				ELSE
					SELECT @ARR_LTV = 70,@LTV_PERC_A = 70, @LTV_PERC=70
			END
		END		
		...
		SELECT		ISNULL(@PrdCode,''),ISNULL(ROUND(@OutSandAmt,0),0),ISNULL(ROUND(@ARR_OutSandAmt,0),0),ISNULL(ROUND(@TopUpAmt,0),0),ISNULL(ROUND(@ARR_TopUpAmt,0),0),ISNULL(ROUND(@BT_EMI,0),0),
					ISNULL(ROUND(@TOPUP_EMI,0),0),ISNULL(ROUND(@FINAL_LOAN_AMT,0),0) , ISNULL(ROUND(@ARR_EMI,0),0) , ISNULL(@FIN_ARR_FOIR,0), ISNULL(@ARR_ROI,0) , ISNULL(@ARR_LTV,0) , ISNULL(@EXPLOAN,0) ,
					ISNULL(@EXP_EMI,0), ISNULL(@EXP_ROI,0) , ISNULL(@TENURE,0) , ISNULL(@CIBIL ,0),ISNULL(@MRKPROP_VAL,0),ISNULL(@INCOME,0),@SALARY_TYPE , @SAL_PROOF , ISNULL(@OBLIGATIONS,0),
					ISNULL(@ACT_PRP_VAL,0),@LTV_PERC, @FOIR, @SHPLR, @EXPLOAN,@LTV_PERC_A,@BT_LTV_A,@TOPUP_LTV_A,@BT_LTV_M,@TOPUP_LTV_M
