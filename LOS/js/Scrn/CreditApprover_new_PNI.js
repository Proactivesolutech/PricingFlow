...
$(document).ready(function () {
    ...
    $(".category-info ul").click(function () {
        // For sub Loan types         
        if (window.CREDIT_APPROVER_NO == 1 || window.CREDIT_APPROVER_NO == 3) {
            ...
            fnCallLOSWebService("SELECT_SUB_PRODUCTS", PrcObj, fnCreditOffResult, "MULTI");
        }
    });
...
function fnSelectSummary() {
    ...
    GlobalXml[0].CreditDocNo = window.CREDIT_DOC_NO;
...
function fnCreditOffResult(ServiceFor, Obj, Param1, Param2) {
    ...
    if (ServiceFor == "SELECT_SUB_PRODUCTS") {
        $("#category-div").show();
        $("#category-div .popup-content").find("ul:nth-child(2)").hide();
        $("#category-div .popup-content").find("ul:nth-child(3)").hide();
        ...
        $(subProductType).find("li").click(function () {
            var PrdPk = Number($(this).find("i").attr("pk")) || 0;
            if (PrdPk != 0) {
                var PrcObj = { ProcedureName: "PrcShflCreditOff_new", Type: "SP", Parameters: ["UPDATE_PRD_PNI", JSON.stringify(GlobalXml), '', PrdPk] };
                fnCallLOSWebService("UPDATE_PRD_PNI", PrcObj, fnCreditOffResult, "MULTI");
            }
        });
    }
    if (ServiceFor == "UPDATE_PRD_PNI") {
        $("#category-div").hide();
        fnSelectSummary();
    }
    ...
    if (ServiceFor == "SELECT_SUMMARY") {
        ...
        setTimeout(function () {
            // FOR PNI CASES          
            if (IS_PNI && CREDIT_APPROVER_NO == 3) {
                var slider = $("#ElgLoanAmt").find("[key=rangeval]").data("ionRangeSlider");
                var value = $("#ElgLoanAmt").find("[key=rangeval]").val();
                if (slider)
                    slider.update({
                        from: value,
                        onChange: fnOnchange,
    ...
    if (ServiceFor == "SELECT_FB_POP") {
        ...
        fnSelectFB_POP_ZCPNI(type, tmpTeledata, pddata, TeleData_Ph);
    }
    /*Changes end By Kani On 20/12/2016*/
    if (ServiceFor == "TAX") {
...
function fnAgentVerfDeviation(AgentData) {
    ...
    for (var i = 0; i < VerfData.length; i++) {
        ...
        var dvCls = Status == 0 ? statusList["N"].class : Status == 1 ? statusList["M"].class : Status == 2 ? statusList["P"].class : 'bg8';
        var ActiveDiv;
        if (serv == "PD") {
...
function fnInputChanged_BT(OBJ) {
    ...
    //$("#Fin_LOAN [key='textval']").html('<i class="icon-indian-rupee"></i>' + LoanAmt_L + '<span class="lakhs">L</span>');    
    ...
    var EMI = Math.round(OBJ.TOPUP_EMI.toFixed(2))
    ...
    var EmiVal = CreditFormulas.PMT(Irate, tenure, totAmt);
    EmiVal = OBJ.BT_EMI || 0;
    $("#BT_Fin_EMI [key='textval']").html('<i class="icon-indian-rupee"></i>' + EmiVal + '');
...
function fnSetCreditInfo_BT(data) {
    ...
    ROI_CHANGE = OBJ.ROI_CHANGE || 0;
    INCOME = OBJ.NET_INC, OBLIGATIONS = OBJ.OBL, ROI = OBJ.ROI, EMI = OBJ.EMI, LOAN_AMT = OBJ.LOAN_AMT, TENURE = OBJ.TENUR,
    MRKPROP_VAL = OBJ.EST_PRP, LTV_PERC = OBJ.LTV, LTV_PERC_A = OBJ.ACT_LTV, AGRPROP_VAL = OBJ.ACT_PRP, IIR = OBJ.IIR,
    FOIR = OBJ.FOIR, CIBIL = OBJ.CBL, LI = OBJ.LI, GI = OBJ.GI, BT_GI = OBJ.BT_GI, BT_LI = OBJ.BT_LI, TOP_GI = OBJ.TOPUP_GI,
    TOP_LI = OBJ.TOPUP_LI, BT_LOAN_AMT = OBJ.BT_AMT, BT_ROI = (OBJ.BT_ROI + ROI_CHANGE), TOPUP_ROI = (OBJ.TOPUP_ROI + ROI_CHANGE),
    TOPUP_AMT = OBJ.TOPUP_AMT, BT_EMI = OBJ.BT_EMI, TOPUP_EMI = OBJ.TOPUP_EMI;
    var SHPLR = 15;
...
function fnSetExpectedValues(ExpData) {
    ...
    //$("#ExpLoanAMt p").html('<i class="icon-indian-rupee"></i>' + exploan + '<span class="lakhs">L</span>');
...
function fnOnchange(data) {
    ...
    if (attr == "LOAN_AMT") {
        ...
        $(input).closest("div.box-sum").find("p[key='textval']").html('<i class="icon-indian-rupee"></i><span class="value" contenteditable="true"> ' + FormatCurrency(data.from));
    }
    if (attr == "TENURE") {
        TENURE = data.from;
...
function fnCreditandSanctionEntry_BT(isMoveNxt, IsReject) {
    ...
    var CreditInput = { LTV: inputLtv, ROI: InputROI, LOAN: InputLoan, FOIR: InputFOIR, TENURE: InputTenure, TENURE_TOP: InputTenure_TOP };

    var OBL, NET_INC, FOIR, CBL, TENUR, LOAN_AMT, ROI, EMI, SPREAD, EST_PRP, ACT_PRP, LTV, ACT_LTV, LI, GI,
    TOPUP_AMT, BT_AMT, TOPUP_ROI, BT_ROI, TOPUP_EMI, BT_EMI, TOPUP_LI, BT_LI, TOPUP_GI, BT_GI , BT_LTV_A, TOPUP_LTV_A, BT_LTV_M, TOPUP_LTV_M;;
    ...
    EST_PRP = Number($("#Prp_ValDiv [key='Mrk']").justtext());
    ACT_PRP = Number($("#Prp_ValDiv [key='Agr']").justtext());
    ...
    var AttrCreditInsert = [{ Attr: "OBL", value: OBL }, { Attr: "NET_INC", value: NET_INC }, { Attr: "CBL", value: CBL }, { Attr: "TENUR", value: TENUR },
       { Attr: "TOPUP_AMT", value: TOPUP_AMT }, { Attr: "BT_AMT", value: BT_AMT }, { Attr: "BT_ROI", value: (BT_ROI + ROI_CHANGE) }, { Attr: "TOPUP_ROI", value: (TOPUP_ROI) },
       { Attr: "BT_EMI", value: BT_EMI }, { Attr: "TOPUP_EMI", value: TOPUP_EMI }, { Attr: "SPREAD", value: SPREAD },
...
function fnCreditandSanctionEntry(isMoveNxt, IsReject) {
    ...
    var OBL, NET_INC, FOIR, CBL, TENUR, LOAN_AMT, ROI, EMI, SPREAD, EST_PRP, ACT_PRP, LTV, ACT_LTV, LI, GI, ROI_CHANGE;
    ...
    var AttrCreditInsert = [{ Attr: "OBL", value: OBL }, { Attr: "NET_INC", value: NET_INC }, { Attr: "CBL", value: CBL }, { Attr: "TENUR", value: TENUR }, { Attr: "LOAN_AMT", value: LOAN_AMT },
        { Attr: "ROI", value: (ROI - ROI_CHANGE) }, { Attr: "EMI", value: EMI }, { Attr: "SPREAD", value: SPREAD },
...
function fnOnPopUpCust(PopObj) {    
    $("#bc_Feedback_div_ZCPNI input[name='text']").val("");
    $("#bc_Feedback_div_ZCPNI textarea[name='text']").val("");
    $("#rptsts_ZCPNI").attr("class", "icon-positive");
    $("#rptstsZCPNI").closest("p.bg").attr("class", "bg bg1");
    $("#bc_Feedback_div_ZCPNI input[key=pop_rptstatus]").val(2);
    $(".contact-details label").text("Details");
    $(".contact-details #ltv_Mob_ZCPNI span").text("");
    $(".contact-details #ltv_Lnd_ZCPNI span").text("");
    $("#PDfileList_ZCPNI").empty();
...
function fnSelectFB_POP_ZCPNI(type, teledata, pddata, TV_Ph) {    
    if (type == 1 || type == 3) {
        $("#mobnum_ZCPNI").show();
        $("#bc_Feedback_div_ZCPNI").attr("notetype", type);
        if (type == 1)
            $("#Feeback_title_ZCPNI").text("Tele Verification - Office");
        else if (type == 3)
            $("#Feeback_title_ZCPNI").text("Tele Verification - Residential");
        if (TV_Ph.length > 0) {
            if (type == 3) {
                $(".contact-details label").text("Contact Details");
                $(".contact-details #ltv_Mob_ZCPNI span").text(TV_Ph[0].LapMob);
                $(".contact-details #ltv_Mob_ZCPNI span").attr("PhNo", TV_Ph[0].LapMob);
                $(".contact-details #ltv_Lnd_ZCPNI span").text(TV_Ph[0].LapRes);
                $(".contact-details #ltv_Lnd_ZCPNI span").attr("PhNo", TV_Ph[0].LapRes);
                var althtml;
                if (TV_Ph[0].alterno != "") {
                    althtml = '<i class="icon-mobile"></i>Alternate No : <span class="TVphNo" PhNo="' + TV_Ph[0].alterno + '">' + TV_Ph[0].alterno + '</span></p>';
                    $(".contact-details #ltv_alter_ZCPNI").html(althtml);
                }
                $(".contact-details #ltv_off_ZCPNI").html("");
            }
            else if (type == 1) {
                $(".contact-details #ltv_alter_ZCPNI").html("");
                $(".contact-details label").text("Contact Details");
                $(".contact-details #ltv_Mob_ZCPNI span").text(TV_Ph[0].LapMob);
                $(".contact-details #ltv_Mob_ZCPNI span").attr("PhNo", TV_Ph[0].LapMob);
                $(".contact-details #ltv_Lnd_ZCPNI span").text(TV_Ph[0].LapRes);
                ...
                $(".contact-details #ltv_off_ZCPNI").html(offHTML);
            }
        }
        ...
        if (teledata.length > 0) {
            if (teledata[0].pop_rptstatus == 0) {
                $("#rptsts_ZCPNI").attr("class", "icon-negative");
                $("#rptsts_ZCPNI").closest("p").attr("class", "bg bg2");
            }
            else if (teledata[0].pop_rptstatus == 1) {
                $("#rptsts_ZCPNI").attr("class", "icon-no-status");
                $("#rptsts_ZCPNI").closest("p").attr("class", "bg bg7");
            }
            else if (teledata[0].pop_rptstatus == 2) {
                $("#rptsts_ZCPNI").attr("class", "icon-positive");
                $("#rptsts_ZCPNI").closest("p").attr("class", "bg bg1");
            }
            $("#spantele").attr("pk", teledata[0].LtvPk);
            $("#bc_Feedback_div_ZCPNI").attr("notepk", teledata[0].LtvPk);
            fnSetValues("bc_Feedback_div_ZCPNI", teledata);
            $("#bc_Feedback_div_ZCPNI").show();
        }
        else {
            $("#bc_Feedback_div_ZCPNI").attr("notepk", 0);
            $("#bc_RptDt_ZCPNI").val("");
            $("#bc_RptRmks_ZCPNI").val("");
        }


        $("#PDattachDiv_ZCPNI").hide();
        $("#bc_Feedback_div_ZCPNI").show();

    }
    else if (type == 2) {
        $("#Feeback_title_ZCPNI").text("Personal Discussion");
        $("#bc_Feedback_div_ZCPNI").attr("notetype", 2);
        $("#mobnum_ZCPNI").hide();

        $("#PDattachDiv_ZCPNI").show();
        ...
        if (pddata.length > 0) {
            ...
            $("#bc_Feedback_div_ZCPNI").attr("notepk", pddata[0].LpdPk);
            ...
            if (DocPath && DocPath != '' && DocPath != null) {
                ...
                $("#PDfileList_ZCPNI").append(docfiles);
            }

            fnSetValues("bc_Feedback_div_ZCPNI", pddata);
            $("#bc_Feedback_div_ZCPNI").show();
...
function fnEditNotes(Notespk, elem, UsrPk) {
    ...
    if (Notespk == 0) {
        ...
        if (UsrPk && Number(UsrPk) != Number(GlobalXml[0].UsrPk)) {
