...
function fnAddNewApplicant(e, elem, pk, AppData) {
    ...
    $(".customer-financial-div[val='" + len + "']").find(".liability-div .finance-tab li h4").html("<i class='icon-indian-rupee'></i> 0 ");
...
function fnBuildApplicantSummary(_data, Info, BankData, Liabilitydata, ApplicantIncomeDetails, AgentVerf) {
    ...
    for (var j = 0; j < data.length; j++) {
        ...
        Summary += '<div box="' + (j+1) + '" AppFk="' + data[j].AppFk + '"  class="applicant-box">' +
                    ...
                    ' <p style="display:none;" class="ApplicantName">' + data[j].ApplicantName + '</p>' +
                    '  <p class="Appincome-type" incType="' + custIncType + '">' + (custIncType == -1 ? "-" : customerClass[custIncType].name) + '</p>' +
                    ...
                    '    <span pk="0" class="bc_link_feeback bg bg1 cursor" flag="3" id="spantele_R">TE (R)</span> '+
                    '    <span pk="0" class="bc_link_feeback bg bg1 cursor" flag="1" id="spantele">TE (O)</span> '+
                    '    <span pk="0" class="bc_link_feeback bg bg1 cursor" flag="2" id="spanpd">PD</span> ' +
                    ' </div>' +
                    ' </div>' +
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (i > 0) {
            ...
            $(".customer-financial ul.common-tabs li:first-child p").text(nm);
            $(".customer-financial ul.common-tabs li:first-child").attr("appPk", data[i].AppFk);
            var actorType = data[i].Actor == 0 ? "Applicant" : data[i].Actor == 1 ? "Co-Applicant" : data[i].Actor == 2 ? "Guarantor" : "";
            $(".customer-financial ul.common-tabs li:first-child .app-type").text(actorType);
            ...
            for (var k = 0; k < BankData.length; k++) {
                if (BankData[k].AppPk == data[i].AppFk) {
                    ...
                    var AccTypeHTML = '     <div name="AccountType" class="select-focus">' +
                                  ...
                                  '      <li val="0">Current</li>' +
                                  '              <li val="1">Saving</li>' +
                                  '             <li val="2">Fixed Deposit</li>' +
                                  '            <li val="3">Overdraft</li>' +
                                  '           <li val="4">Cash Credit</li>' +
                                  '        </ul>' +
                                  '     </div>';
...
function fnAddIncomeColumn(elem, classNm) {
    ...
    $(IncomeDiv).find("[restrict='number']").keyup();
    return retDiv;
}
...
function fnCustFinResult(ServiceFor, Obj, Param1, Param2) {
    ...
    if (ServiceFor == "UPDATE_APP_EMP") {
        ...
        if (!Data || Data.length == 0) {
            ...
            setTimeout(function () { $("#btn_Ok").click(); }, 600);
            return;
        }
        var Info = Data[0];
        if (Info.ProductCode == "HLBTTopup" || Info.ProductCode == "HLBT" || Info.ProductCode == "HLTopup" ||
        ...
        if (Info.ProductCode.toUpperCase() == "HLTOPUP" || Info.ProductCode.toUpperCase() == "HLEXT" || Info.ProductCode.toUpperCase() == "HLIMP" || Info.ProductCode.toUpperCase() == "LAPTOPUP") {
            RefUl += ' <comp-help id="comp-help" txtcol="RefLoanNo" valcol="Loanpk" onrowclick="Loanclick" prcname="PrcShflRefNohelp" width="100%" LoanPk = "0" LoanNo = ""></comp-help>' 
    ...
    if (ServiceFor == "DOCUMENT") {
        ...
        var doc_data = JSON.parse(Obj.result);
    ...
    $(".customer-financial-div").each(function () {
        ...
        $(BankUL).each(function () {
            ...
            var BankObj = {
                ...
                BankFk: BankFk, BranchFk: BranchFk
            }
        ...
        $(LiabilityUL).each(function () {
            ...
            var LiabilityObj = {
                ...
                incexlswitch: incexlswitch, svsswitch: svsswitch, OblTyp: OblTyp, BounceNo: Bounce, DepBank: DepBnk
            }
...
function fnSaveFinancialData(FromCO) {
    ...
    var PrcObj = { ProcedureName: "PrcShflCustFinancials", Type: "SP", Parameters: ["INSERT_FIN_DET", JSON.stringify(CustFinGlobal), JSON.stringify(detailJSON)] };
...
function fnGetIncomeHead() {
    ...
    try {
        $(".customer-financial-div").each(function () {
            ...
            var lbtyAmt = FormatCleanComma($(liabtyDiv).find("ul.finance-tab li.obligation-li h4").justtext());
            var obj = {};
            obj.LapFk = LapFk;
            obj.incometype = 'OB';
...
function fnSetIncomeComponent(div) {
    ...
    $(div).find("input[class^=range][isperc=1]").closest(".income-range").find(".percText").remove()
    var percInputs = $(div).find("input[class^=range][isperc=1]");
...
function fnpopupconfirm() {
    ...
    fnCallLOSWebService(Action, objProcData, fnCustFinResult, "MULTI", "");
}
    ...
    if (type == 1 || type == 3) {
        ...
        if (TV_Ph.length > 0) {
            if (type == 3) {
                ...
                $(".contact-details #ltv_Mob span").attr("PhNo", TV_Ph[0].LapMob);
                if (TV_Ph[0].LapRes == "") {
...
function fnEventWithDocumentTarget() {
    window.IsBranCreditLoaded = false;
    if (!window.IsBranCreditLoaded && window.IsBranCreditLoaded != true) {
        var defincnm = 1;
        ...
        window.IsBranCreditLoaded = true;
    }
}
function fnpop() {
...
function fnIncomeKeyUp(e, elem) {
    ...
    try {
        ...
        if ($(UlList).find("li").length == 1) { }
            ...
            var totalObligation = 0, totalIncome = 0;
...
function fnAddExitingLoanRow() {
    ...
    var RefUl = '';
    if (PrdCode.toUpperCase() == "HLTOPUP" || PrdCode.toUpperCase() == "HLEXT" || PrdCode.toUpperCase() == "HLIMP" || PrdCode.toUpperCase() == "LAPTOPUP") {
        RefUl += ' <comp-help id="comp-help" class="RefNo" txtcol="RefLoanNo" valcol="Loanpk" onrowclick="Loanclick" prcname="PrcShflRefNohelp" width="100%" LoanPk = "0" LoanNo = ""></comp-help>'                  
    } else {
        RefUl += '<input label="Reference Number" class="RefNo" type="text" placeholder="">'
    }
    var ul = ' <ul class="form-controls">' +
...
function fnAddExistLn(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var RefUl = '';
        if (PrdCode.toUpperCase() == "HLTOPUP" || PrdCode.toUpperCase() == "HLEXT" || PrdCode.toUpperCase() == "HLIMP" || PrdCode.toUpperCase() == "LAPTOPUP") {
            RefUl += ' <comp-help id="comp-help" txtcol="RefLoanNo" valcol="Loanpk" onrowclick="Loanclick" prcname="PrcShflRefNohelp" width="100%" LoanPk = "0" LoanNo = "' + data[i].RefNo + '"></comp-help>'
            ...
            PropUl += '<div class="div-left grid-50">' +
                    ...
                    ' <li class="width-21">' +
                        '<label>Nearest Branch</label>' +
                                ...
                                ' <comp-help id="comp-help" txtcol="Location" valcol="Brnchpk" helpfk="0" onrowclick="Bc_Brnchclick" prcname="PrcShflleadBranchhelp" width="100%"></comp-help>' +
...
function fnShowManDevPop() {
    $('#BCDeviationLvl').val(0);
    $('#BCDeviationRmks').val("");

    var PrcObj = { ProcedureName: "PrcShflCustFinancials", Type: "SP", Parameters: ["SELECT_MANUALDEV", JSON.stringify(CustFinGlobal), "", "", "", "", "", ""] };
    fnCallLOSWebService("SELECT_MANUALDEV", PrcObj, fnCustFinResult, "MULTI");    
}


function fnSaveManualDevBC() {
    var ApprveLvl = $('#BCDeviationLvl').val();
    var DevRmks = $('#BCDeviationRmks').val() || "";
    DevRmks = DevRmks.trim();
    if (ApprveLvl == 0 || ApprveLvl == "") {
        fnShflAlert("warning", "Choose Approver Level.");
        return;
    }
    if (DevRmks == "") {
        fnShflAlert("warning", "Remarks is mandatory.");
        return;
    }
    CustFinGlobal[0].ApprveLvl = ApprveLvl;
    CustFinGlobal[0].DevRmks = DevRmks;
    var PrcObj = { ProcedureName: "PrcShflCustFinancials", Type: "SP", Parameters: ["ADD_MANUALDEV", JSON.stringify(CustFinGlobal), "", "", "", "", "", ""] };
    fnCallLOSWebService("ADD_MANUALDEV", PrcObj, fnCustFinResult, "MULTI");

    CustFinGlobal[0].ApprveLvl = "";
    CustFinGlobal[0].DevRmks = "";
}
